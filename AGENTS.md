...
在 `autonomous` 下，用户对低风险范围说“同意 / 可以 / 我确认 / 继续做”以后，不需要每个内部阶段都再让用户说一次“继续”。Codex 应在 active boundary 内继续推进本地可证明的实现、验证、文档同步、门禁恢复和必要的 subagent 分工；一个低风险任务收口后，如果 phase plan、交付总结或下一步建议里的下一项仍是本地可验证低风险任务，Codex 应自动创建或激活下一段 boundary 并继续推进。只有遇到业务多解、真实数据、生产、数据库、破坏性动作、git workflow action、验证失败或证据缺口时才暂停确认。
...
7. 最终交付回复必须包含下一步建议；可以是验收、处理反馈、补剩余风险、进入下一轮修改或转入下一个优先任务。下一步建议不是默认暂停点：如果下一项仍是低风险本地任务，说明 Codex 会继续；如果涉及业务口径、多解方案、真实数据、生产、数据库、破坏性命令或 git workflow action，才明确等待用户确认。
