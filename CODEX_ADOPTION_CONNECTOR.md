...
- `upgrade-plan`：面向已有项目生成增量升级计划，说明哪些能力已具备、哪些可安全应用、哪些需要迁移或合并计划。
- `upgrade-apply`：只执行安全幂等改造，例如创建缺失的 `stack/<adapter>/`；不覆盖 adapter，不移动业务代码。
...
- 已有项目升级入口：用户说“按最新版骨架升级当前项目”，Codex 先生成增量升级计划，不要求新建项目重新接入。
