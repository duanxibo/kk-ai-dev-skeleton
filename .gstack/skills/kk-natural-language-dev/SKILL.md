...
- For most nontechnical utterances, Codex should first run `python3 .gstack/scripts/nontechnical_next_step.py --raw "..." --format user` internally and use that as the draft user-facing reply.
  ...
  - If the user says "进行 UI 优化", "优化界面", "美化页面", "页面不好看", "界面太丑", "提升视觉", or equivalent wording, this helper may route to `nontechnical_ui_optimization.py` and explain that Codex will automatically run UI design kickoff, implementation, polish review, and browser acceptance without asking the user to name internal skills.
