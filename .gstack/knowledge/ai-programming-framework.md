...
低风险阶段必须可自动串联：一个低风险任务收口后，如果 phase plan、交付总结或下一步建议中的下一项仍是本地可验证低风险任务，Codex 应自动创建或激活下一段最小 boundary 并继续推进；每段仍必须有范围、验收口径和 QA evidence。遇到业务口径、产品 / 设计多解、真实数据、生产、数据库、破坏性命令、代码提交流程、验证失败或证据缺口时才停下来问用户。
...
- 低风险任务完成后可以自动串联下一段低风险任务，不把下一步建议变成默认停顿点
- 暂停、继续和撤销请求可控
- 范围变化不会导致整个任务重来
...
- delivery summary 的下一步建议会区分低风险自动继续、用户决策和高风险授权
- 新经验会回写到仓库
