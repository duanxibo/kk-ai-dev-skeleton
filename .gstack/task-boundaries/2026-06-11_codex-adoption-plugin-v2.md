# codex-adoption-plugin-v2 Task Boundary

- Task: codex-adoption-plugin-v2
- 负责人: Codex
- 日期: 2026-06-11
- 相关 issue / doc: `.gstack/requirements/2026-06-11_codex-adoption-plugin-v2-fast-lane-requirement.md`

## Goal

- 创建 repo-local Codex 接入器 plugin 骨架，包装 V1 内部安装器，让公司内部后续可产品化分发

## Allowed Files

- .gstack/requirements/2026-06-11_codex-adoption-plugin-v2-fast-lane-requirement.md
- .gstack/reviews/2026-06-11_codex-adoption-plugin-v2-fast-lane-review.md
...
- 已有 V1 helper 行为：`scripts/init_project.py`
...
- 仓库内存在可验证的 repo-local Codex plugin 源目录：`plugins/kk-dev-skeleton-adoption/`。
- plugin manifest 使用规范的 `.codex-plugin/plugin.json`，且无 TODO 占位符。
- plugin 内提供 skill，说明如何通过自然语言接入 KK Dev Skeleton，并调用目标仓库的 V1 helper。
- 文档说明 V2 当前是源目录 / 分发骨架，不是已安装 marketplace。
- 业务用户入口仍是对 Codex 说自然语言，不要求手动执行脚本。

## Generated Artifact Policy

- User-visible Change: `no`
- Artifact Type: `repo-local plugin source and markdown docs`
- Acceptance Surface: `plugin manifest, skill files, validation output`
- Browser QA: `not-required`
- Reason:
  本次不生成 HTML、页面、dashboard、截图或可交互 UI；验收面是 plugin 文件结构和 validator。
...
- Candidate Subagents:
  - name: `not-used`
    ...
    forbidden paths:
      - `stack/**`
    output evidence: `.gstack/qa-reports/2026-06-11_codex-adoption-plugin-v2-qa.md`
    status: `not-used`
- Result Integration:
...
- requirement-brief:
  status: done
  evidence: `.gstack/requirements/2026-06-11_codex-adoption-plugin-v2-fast-lane-requirement.md`
- plan-ceo-review:
  status: done
  evidence: `.gstack/reviews/2026-06-11_codex-adoption-plugin-v2-fast-lane-review.md`
- requirement-freeze:
  status: done
  evidence: `.gstack/requirements/2026-06-11_codex-adoption-plugin-v2-fast-lane-requirement.md`
...
- domain-spec-readiness:
  status: not-required
  evidence: `.gstack/task-boundaries/2026-06-11_codex-adoption-plugin-v2.md`
  note: 本次 fast-lane 小需求不改变 stack domain 产品或工程真源。
- implement:
  status: done
  evidence: `plugins/kk-dev-skeleton-adoption/.codex-plugin/plugin.json`
- qa:
  status: done
  command: qa-only
  evidence: `.gstack/qa-reports/2026-06-11_codex-adoption-plugin-v2-qa.md`
...
required_gates:
  - gate_id: data-access
    ...
    status: not-required
    evidence_path: ".gstack/task-boundaries/2026-06-11_codex-adoption-plugin-v2.md"
