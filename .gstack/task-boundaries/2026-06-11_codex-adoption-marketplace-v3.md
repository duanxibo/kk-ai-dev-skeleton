# codex-adoption-marketplace-v3 Task Boundary

- Task: codex-adoption-marketplace-v3
- 负责人: Codex
- 日期: 2026-06-11
- 相关 issue / doc: `.gstack/requirements/2026-06-11_codex-adoption-marketplace-v3-fast-lane-requirement.md`

## Goal

- 创建公司内部 marketplace 源和安装升级策略，连接 V2 plugin 源目录，但不在当前环境执行安装

## Allowed Files

- .gstack/requirements/2026-06-11_codex-adoption-marketplace-v3-fast-lane-requirement.md
- .gstack/reviews/2026-06-11_codex-adoption-marketplace-v3-fast-lane-review.md
...
- 当前机器 Codex config 或安装状态
- V1 helper 行为：`scripts/init_project.py`
- V2 plugin manifest / skill 内容，除非验证发现必要修复
...
- 仓库内存在 repo-local marketplace source：`.agents/plugins/marketplace.json`。
- marketplace entry 指向 `./plugins/kk-dev-skeleton-adoption`，并包含 installation、authentication 和 category。
- 文档说明如何由管理员或 Codex 在明确授权后安装 repo marketplace。
- 文档说明如何升级 plugin：先改 plugin 源，再校验，再通过 marketplace 重装或新会话验证。
- 文档持续强调：业务用户入口仍是自然语言接入，不是命令行安装。

## Generated Artifact Policy

- User-visible Change: `no`
- Artifact Type: `repo-local marketplace source, install guide, tests, markdown docs`
- Acceptance Surface: `marketplace JSON, plugin validation, unit tests, guard output`
- Browser QA: `not-required`
- Reason:
  本次不生成 HTML、页面、dashboard、截图或可交互 UI；验收面是 marketplace 文件、安装说明和 validator。
...
- Candidate Subagents:
  - name: `not-used`
    ...
    forbidden paths:
      - `stack/**`
    output evidence: `.gstack/qa-reports/2026-06-11_codex-adoption-marketplace-v3-qa.md`
    status: `not-used`
- Result Integration:
...
- requirement-brief:
  status: done
  evidence: `.gstack/requirements/2026-06-11_codex-adoption-marketplace-v3-fast-lane-requirement.md`
- plan-ceo-review:
  status: done
  evidence: `.gstack/reviews/2026-06-11_codex-adoption-marketplace-v3-fast-lane-review.md`
- requirement-freeze:
  status: done
  evidence: `.gstack/requirements/2026-06-11_codex-adoption-marketplace-v3-fast-lane-requirement.md`
...
- domain-spec-readiness:
  status: not-required
  evidence: `.gstack/task-boundaries/2026-06-11_codex-adoption-marketplace-v3.md`
  note: 本次 fast-lane 小需求不改变 stack domain 产品或工程真源。
- implement:
  status: done
  evidence: `.agents/plugins/marketplace.json`
- qa:
  status: done
  command: qa-only
  evidence: `.gstack/qa-reports/2026-06-11_codex-adoption-marketplace-v3-qa.md`
...
required_gates:
  - gate_id: data-access
    ...
    status: not-required
    evidence_path: ".gstack/task-boundaries/2026-06-11_codex-adoption-marketplace-v3.md"
...
- `rg --hidden -n "codex plugin marketplace add|codex plugin add" CODEX_ADOPTION_CONNECTOR.md COMPANY_ADOPTION_GUIDE.md README.md plugins/MARKETPLACE_INSTALL.md .agents/plugins/README.md -S`
